    ...
    internal partial class Binder
    {
        ...
        public virtual BoundStatement BindStatement(StatementSyntax node, BindingDiagnosticBag diagnostics)
        {
            if (node.AttributeLists.Count > 0)
            {
                ...
                if (node.Kind() is not SyntaxKind.LocalFunctionStatement or SyntaxKind.Block)
                {
            ...
            Debug.Assert(Enumerable.Contains(result.Syntax.AncestorsAndSelf(), node), @"Bound statement (or one of its parents)
        ...
        {
            var usingDeclaration = UsingStatementBinder.BindUsingStatementOrDeclarationFromParts(node, node.UsingKeyword, node.AwaitKeyword, originalBinder: this, usingBinder: null, diagnostics);
        ...
        private BoundStatement BindDeclarationStatementParts(LocalDeclarationStatementSyntax node, BindingDiagnosticBag diagnostics)
        {
            ...
            // Ideally, this would be in the constructor of SourceLocalSymbol, but it lacks a diagnostics bag passed to it to add this diagnostic.
            ...
            out ExpressionSyntax value)
        {
            ...
            value = initializer?.Value.UnwrapRefExpression(diagnostics, out expressionRefKind);
        ...
        private BoundExpression BindAssignment(AssignmentExpressionSyntax node, BindingDiagnosticBag diagnostics)
        {
            ...
            var rhsExpr = node.Right.UnwrapRefExpression(diagnostics, out var refKind);
...
#nullable disable
        ...
        private BoundStatement BindReturn(ReturnStatementSyntax syntax, BindingDiagnosticBag diagnostics)
        {
            ...
            var expressionSyntax = syntax.Expression?.UnwrapRefExpression(diagnostics, out refKind);
            ...
            BindingDiagnosticBag diagnostics)
        {
            ...
            {
                ExpressionSyntax expressionSyntax = expressionBody.Expression.UnwrapRefExpression(diagnostics, out RefKind refKind);
        ...
        public BoundBlock BindLambdaExpressionAsBlock(ExpressionSyntax body, BindingDiagnosticBag diagnostics)
        {
            ...
            var expressionSyntax = body.UnwrapRefExpression(diagnostics, out RefKind refKind);
            BoundExpression expression = bodyBinder.BindValue(expressionSyntax, diagnostics, GetRequiredReturnValueKind(refKind));
